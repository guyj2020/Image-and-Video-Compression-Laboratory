function [motion_vectors_indices, ssd] = SSD8x16(ref_image, image)
...
ref_image = padarray(ref_image, [4, 8], 'both', 'replicate');
motion_vectors_indices = blockproc(image, [8, 16], @(block_struct) BlockSSDMV(block_struct.data, block_struct.location, ref_image));
ssd = blockproc(image, [8, 16], @(block_struct) BlockSSD(block_struct.data, block_struct.location, ref_image));
